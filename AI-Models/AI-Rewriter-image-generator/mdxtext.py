...
def generate_unique_article(article):
    ...
    prompt = f"""
    You're a professional journalist. Rewrite the article below to be unique and format it in MDX (Markdown with JSX support). Use standard Markdown syntax (e.g., # for headings, - for lists, > for quotes) and ensure the output is clean and structured for a blog post. If appropriate, include simple MDX components like <p> or <div> for sections, but keep it simple and readable.

    **Original Article:**
    Headline: {article["headline"]}
    Content: {article["content"][:200]}  # Reduce to 200 chars for faster processing
    ...
    **MDX Output:**
    ...
    try:
        inputs = tokenizer(prompt, return_tensors="pt", truncation=True, max_length=128).to(device)  # Reduce max_length
        print("🔧 Rewriting and formatting article into MDX...")
        logging.info(f"Starting text generation with input length: {len(prompt)} chars")

        max_new_tokens = 200  # Increase slightly for MDX formatting
        with torch.cuda.amp.autocast():  # Use mixed precision for speed
            with tqdm(total=max_new_tokens, desc="Generating MDX", unit="token") as pbar:
                output = text_model.generate(
                    **inputs,
                    ...
                    max_time=60,  # Prevent infinite loops
                )
        ...
        if "**MDX Output:**\n```" in generated_text:
            generated_text = generated_text.split("**MDX Output:**\n```")[-1].strip().rstrip("```")
        else:
            logging.warning("⚠️ MDX section not found in output. Full output: " + generated_text)
            generated_text = generated_text  # Fallback to full output
...
def process_new_files():
    ...
    while True:
        try:
            ...
            if not files:
                print("🔍 No new files found. Checking again in 2 seconds...")
                time.sleep(2)
                continue
            ...
            for file in files:
                ...
                for index, article in enumerate(articles, start=1):
                    ...
                    article_filename = f"reworded_{timestamp}_{index}.mdx"  # Change extension to .mdx
                    article_path = os.path.join(ARTICLE_OUTPUT_DIR, article_filename)

                    with open(article_path, "w", encoding="utf-8") as f:
                        json.dump({
                            "headline": rewritten_text.split("\n")[0] if "\n" in rewritten_text else rewritten_text,
                            "url": article.get("url", "N/A"),
                            "source": article.get("source", "N/A"),
                            "content": rewritten_text,  # MDX-formatted content
                            "image_prompt": ai_prompt,
                            "image_filename": image_filename
                        }, f, indent=4)
                    
                    print(f"✅ Article saved as MDX: {article_path}")
